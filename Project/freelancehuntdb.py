...
DB_NAME = 'projects.db'
HOST = 'https://freelancehunt.com/ua'
...
def get_content(html):
    ...
    items = soup.find_all('td', class_='left')
    works = []

    for item in items:
        title_elem = item.find('a', class_='biggest visitable')
        if title_elem:
            title = title_elem.get_text(strip=True)
            link = title_elem.get('href')
            description_elem = item.find('p', style='word-break: break-word')
